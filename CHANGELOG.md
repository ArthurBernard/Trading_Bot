...
- **Engine reads a real dccd store synchronously.** `_make_client` primes the dccd `Client`'s store/registry (the read-only half of dccd's async `__aenter__`), so the feed's sync `read()` works from inside the async step — it previously raised `Client must be used inside 'async with Client()'` (dccd API drift), starving every real-dccd read. Verified against 4.66M real 1m bars. (#PR)
