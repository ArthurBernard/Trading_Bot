...
# @Last modified time: 2019-05-03 16:53:05
...
class ResultManager:
    ...
    ----------
    path, name : str
        Path and name of the file to load and save results.
    ...
    def __init__(self, name, path='.', init_vol=1., period=252):
        self.name = name
        if path[-1] != '/':
            path += '/'
        ...
        self.period = period
        self.df = get_df(path, name + 'result_hist', ext='.dat')
    ...
    def save_result_hist(self):
        ...
        save_df(self.df, self.path, self.name + 'result_hist', ext='.dat')
